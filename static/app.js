document.addEventListener('DOMContentLoaded', () => {
    ...
    let requestFrequencyChartInstance = null;
    let historyCurrentPage = 1;
    const historyRowsPerPage = 6;
    ...
    function renderRequestHistory(historyToRender) {
        ...
        const paginatedItems = historyToRender.slice(startIndex, endIndex);
        ...
        historyContainer.innerHTML = paginatedItems.map(req => {
        ...
        renderPaginationControls(historyToRender.length);
    }
    ...
    function renderPaginationControls(totalItems) {
        ...
        if (totalPages > 1) {
            ...
            nextPageBtn.disabled = historyCurrentPage === totalPages;
        } else {
    ...
    nextPageBtn.addEventListener('click', () => {
        const totalPages = Math.ceil(requestHistory.filter(req => showDeletedCheckbox.checked || req.status !== 'deleted').length / historyRowsPerPage);
    ...
    window.printRequest = async (controlNumber) => {
        ...
        let pghLogoUrl = '';
        try {
            const response = await fetch('/static/UP_PGH_logo.png');
    ...
    async function loadTopItemsChart() {
        try {
            const response = await fetch('/dashboard/top-items');
            if (!response.ok) throw new Error('Failed to fetch top items data');
            
            const data = await response.json();

            const canvasId = 'topItemsChart';
            const canvas = document.getElementById(canvasId);
            if (!canvas) return;
            ...
            const chartTitleEl = document.getElementById('topItemsChartTitle');
            if(chartTitleEl) {
                chartTitleEl.textContent = `Top 10 Most Requested Items | ${currentMonthName}`;
            }

            if (!data || data.length === 0) {
                displayNoDataMessage(canvasId);
                return;
            }

            const labels = data.map(item => {
                const words = item.item_description.split(' ');
                return words.length > 5 ? words.slice(0, 5).join(' ') + '...' : item.item_description;
            });
            const requestCounts = data.map(item => item.request_count);

            topItemsChartInstance = new Chart(canvas.getContext('2d'), {
                type: 'bar',
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Number of Times Requested',
                        data: requestCounts,
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) { return `Times Requested: ${context.raw}`; },
                                title: function(context) {
                                    const originalIndex = context[0].dataIndex;
                                    return data[originalIndex].item_description;
                                }
                            }
                        }
                    }
            ...
            displayNoDataMessage('topItemsChart');
        }
    }

    async function loadRequestFrequencyChart() {
        try {
            const response = await fetch('/dashboard/request-frequency');
            if (!response.ok) throw new Error('Failed to fetch request frequency data');
            
            const data = await response.json();
            
            const canvasId = 'requestFrequencyChart';
            const canvas = document.getElementById(canvasId);
            if (!canvas) return;

            if (requestFrequencyChartInstance) {
                requestFrequencyChartInstance.destroy();
            }

            const allDates = [];
            const dateMap = new Map(data.map(item => [item.date, {
            ...
            const requestCounts = allDates.map(date => (dateMap.get(date) || { request_count: 0 }).request_count);
            const itemCounts = allDates.map(date => (dateMap.get(date) || { item_count: 0 }).item_count);

            const hasData = itemCounts.some(count => count > 0) || requestCounts.some(count => count > 0);
            if (!hasData) {
                displayNoDataMessage(canvasId);
                return;
            }

            requestFrequencyChartInstance = new Chart(canvas.getContext('2d'), {
                type: 'bar',
                data: {
            ...
            console.error('Error loading request frequency chart:', error);
            displayNoDataMessage('requestFrequencyChart');
        }
    }
